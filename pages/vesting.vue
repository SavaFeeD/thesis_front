          ...
          <div class="card-header">
            ...
            >
              <img
                ...
                class="icon"
              >
              <span>{{ $t("vesting.buttons.add") }}</span>
            </button>
          </div>
              ...
              >
                ...
                >
                  <template #default="{row}">
                    <div
                      v-if="column.prop === 'interval' || column.prop === 'percentage'"
...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        {
          prop: 'number',
          label: this.$t('vesting.columns.number.label'),
          minWidth: 8,
          sortable: true,
        },
        {
          prop: 'start',
          label: this.$t('vesting.columns.start.label'),
          minWidth: 25,
          sortable: true,
        },
        {
          prop: 'interval',
          label: this.$t('vesting.columns.intervals.label'),
          digit: this.$t('vesting.columns.intervals.digit'),
          minWidth: 25,
          sortable: true,
        },
        {
          prop: 'percentage',
          label: this.$t('vesting.columns.percentage.label'),
          digit: this.$t('vesting.columns.percentage.digit'),
          minWidth: 25,
          sortable: true,
        },
        {
          prop: 'edit',
          label: this.$t('vesting.columns.edit.label'),
          minWidth: 17,
          sortable: false,
        },
      ],
      localization: {
        inputs: {
          search: {
            placeholder: this.$t('vesting.search.placeholder'),
          },
        },
      },
    };
  ...
  computed: {
    ...mapGetters({
      tableData: 'vesting/getVestingList',
    }),
  },
  methods: {
    openAddModal() {
      this.ShowModal({
        key: 'vesting-add-modal',
        title: this.$t('vesting.modals.add.title'),
      });
    },
    openEditModal(payload) {
      this.ShowModal({
        key: 'vesting-edit-modal',
        title: this.$t('vesting.modals.edit.title'),
        id: payload,
      });
    },
  },
