              ...
              >
                ...
                >
                  <template #default="{row}">
                    <div v-if="column.prop === 'number'">
                      {{ index + 1 }}
                    </div>
                    <div
                      v-if="column.prop === 'createdAt'"
                    ...
                    <div
                      v-else-if="column.prop === 'inWhitelist'"
                      class="align-items-center d-flex flex-row"
                    >
                      <input
                        ...
                        :value="row"
                      >
                      <label :for="`whitelist-box${row.id}`" @click="changeWhitelistStatus(row)">
                        <div
                          v-if="row.inWhitelist"
...
export default {
  ...
  async mounted () {
    ...
    this.users = users
    this.users.totalCount = users.length
  ...
  methods: {
    ...
    async fetchUsers () {
      return await this.$store.dispatch('admin/fetchUsers')
    },

    async changeWhitelistStatus (item) {
      item.inWhitelist = !item.inWhitelist
      await this.$store.dispatch('admin/changeWhitelistStatus', { id: item.id })
    },
